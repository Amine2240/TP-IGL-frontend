...
export class FormpatientComponent {
  ...
  public async submitForm(): Promise<void> {
    if (this.patientForm.valid) {
      const dpiPayload = {
        // so it will matches the dpi patient serializer
        patient: {
          user: {
      ...
      await this.dpiservice
        ...
        .then((dpi) => {
          ...
          if (dpi?.dpi?.patient?.id) {
            ...
            this.dpiservice
              .ajouterPhotoPatient(patientId, this.photoPatientFile)
              .then((response) => {
                console.log('Photo uploaded successfully:', response);
                const redirectPage =
                  this.globalService.pageToRedirect || '/pageLanding';
    ...
    if (fileInput?.files && fileInput.files[0]) {
      ...
      reader.onload = () => {
        if (type == 'patient') {
          ...
          this.patientForm
            .get('photoPatient')
            ?.setValue(this.photoPatientPreview);
        }
        // if (type == 'codeQr') {
  ...
  onCheckboxChange(
    ...
    value: Item | Allergie | Maladie | Item2,
  ): void {
    ...
    if (isChecked) {
      ...
      const index = formArray.controls.findIndex(
        (control) =>
          control.value.nom === antecedent.nom &&
          control.value.type === antecedent.type,
      );
      if (index !== -1) {
